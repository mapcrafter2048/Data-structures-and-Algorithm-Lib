...
bool DetectCycle(const vector<vector<int>> &graph)
{
        ...
        if (visitedEdges.find(v) == visitedEdges.end())
        {
            ...
            while (!q.empty())
            {
                ...
                for (int w : graph[u])
                {
                    ...
                    if (find(visitedEdges[u].begin(), visitedEdges[u].end(), w) != visitedEdges[u].end())
                    {
                    ...
                    {
                        // Edge (u, w) has already been visited, indicating a cycle
    ...
    for (int u = 0; u < graph.size(); ++u)
    {
        for (int v : graph[u])
        {
            ...
            if (find(visitedEdges[u].begin(), visitedEdges[u].end(), v) == visitedEdges[u].end())
            {
