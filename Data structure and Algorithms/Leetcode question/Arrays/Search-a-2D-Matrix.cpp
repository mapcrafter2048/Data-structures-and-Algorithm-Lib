...
bool searchMatrix(vector<vector<int>> matrix, int target) {
    ...
    while (top <= bottom) {
        ...
        if (matrix[midRow][0] <= target && target <= matrix[midRow][n - 1]) {
            ...
            return false;
        } else if (matrix[midRow][0] >
                   target) { // the smallest element in that particluar row is
                             // greater than the target so the element must be
                             // some where up
            bottom = midRow - 1;
        } else {
            top = midRow +
                  1; // the largest element in that particluar row is less than
                     // the target so the element must be some where down
        }
    }
}
