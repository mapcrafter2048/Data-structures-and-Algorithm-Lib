  ...
  public:
    ...
    int dfs(TreeNode *root, int &ans) {
        if (!root)
            return 0;
        int left = max(
            0, dfs(root->left, ans)); // if there is a negative path from the
                                      // left subtree, we will not consider it
        int right = max(
            0, dfs(root->right, ans)); // if there is a negative path from the
                                       // right subtree, we will not consider it
