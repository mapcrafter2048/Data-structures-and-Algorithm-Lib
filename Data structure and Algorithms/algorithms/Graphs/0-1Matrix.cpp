...
/// @brief The basic intution of the algorithm is to first mark all the '1's as visited and all the other cells that is the cells with '0's as unvisited. We then push all the visited cells into a queue along side their steps that is the steps required to reach nearest 1 and since they already are 1 so hence the step is 0 we then do a BFS on the matrix and keep on updating the distance of the cells from the nearest 1 and then return the distance matrix
